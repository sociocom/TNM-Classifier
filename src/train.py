...
class SingleLabelBERTTrainer():
    ...
    def training_cv(self, df, label_name, cv, batch_size=32, epochs=10, learning_rate=2e-5, max_length=512, early_stopping_rounds=5, label2id=None):
        ...
        for i, (idx_train, idx_eval) in enumerate(cv):
            ...
            train_dataset = SingleLabelDataset(self.tokenizer, texts=train_folds['text'].to_list(), label=train_folds[label_name].to_list(), max_length=max_length, label2id=label2id)
            eval_dataset = SingleLabelDataset(self.tokenizer, texts=eval_folds['text'].to_list(), label=eval_folds[label_name].to_list(), max_length=max_length, label2id=label2id)
            self.train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
            self.eval_loader = DataLoader(dataset=eval_dataset, batch_size=batch_size, shuffle=False)

            self.model = BERTSingleLabelModel(model_name=self.model_name, output_dim=np.unique(cv_labels).size).to(self.device)
    ...
    def training(self, df, label_name, batch_size=32, epochs=10, learning_rate=2e-5, max_length=512, label2id=None):
        train_dataset = SingleLabelDataset(self.tokenizer, texts=df['text'].to_list(), label=df[label_name].to_list(), max_length=max_length, label2id=label2id)
        self.train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
        self.model = BERTSingleLabelModel(model_name=self.model_name, output_dim=df[label_name].nunique()).to(self.device)
        self.optimizer = AdamW(self.model.parameters(), lr=learning_rate)
        for epoch in range(epochs):
            train_result = self._train_epoch()
    ...
    def inferencing(self, df, model, batch_size=32, max_length=512, label2id=None):
        ...
        with torch.no_grad():
            for step, batch in enumerate(test_loader):
                ...
                preds = np.concatenate((preds, output.cpu().detach().numpy().argmax(axis=1)))
    ...
    def _train_epoch(self):
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            train_preds = np.concatenate((train_preds, output.cpu().detach().numpy().argmax(axis=1)))

        train_f1 = f1_score(train_labels, train_preds, average='macro')
        train_acc = accuracy_score(train_labels, train_preds)

        return {'loss': train_loss, 'f1': train_f1, 'accuracy': train_acc, 'labels': train_labels, 'preds': train_preds}
    ...
    def _eval_epoch(self):
        ...
        with torch.no_grad():
            for step, batch in enumerate(self.eval_loader):
                ...
                eval_labels = np.concatenate((eval_labels, label.cpu().detach().numpy()))
                eval_preds = np.concatenate((eval_preds, output.cpu().detach().numpy().argmax(axis=1)))

            eval_f1 = f1_score(eval_labels, eval_preds, average='macro')
            eval_acc = accuracy_score(eval_labels, eval_preds)

            return {'loss': eval_loss, 'f1': eval_f1, 'accuracy': eval_acc, 'labels': eval_labels, 'preds': eval_preds}
...
class MultiLabelBERTTrainer():
    ...
    def training_cv(self, df, cv, batch_size=32, epochs=10, learning_rate=2e-5, max_length=512, early_stopping_rounds=5, label2id=None):
        cv_labelsT, cv_labelsN, cv_labelsM = df['T'].values, df['N'].values, df['M'].values
        cv_predsT, cv_predsN, cv_predsM = np.zeros((len(df))), np.zeros((len(df))), np.zeros((len(df)))
        finished_epochs = []
        for i, (idx_train, idx_eval) in enumerate(cv):
            train_folds = df.loc[idx_train].reset_index(drop=True)
            eval_folds = df.loc[idx_eval].reset_index(drop=True)
            train_dataset = MultiLabelDataset(self.tokenizer, texts=train_folds['text'].to_list(), labelT=train_folds['T'].to_list(), labelN=train_folds['N'].to_list(), labelM=train_folds['M'].to_list(), max_length=max_length)
            eval_dataset = MultiLabelDataset(self.tokenizer, texts=eval_folds['text'].to_list(), labelT=eval_folds['T'].to_list(), labelN=eval_folds['N'].to_list(), labelM=eval_folds['M'].to_list(), max_length=max_length)
            self.train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
            self.eval_loader = DataLoader(dataset=eval_dataset, batch_size=batch_size, shuffle=False)
        ...
        cv_predsTNM = [str(int(t)) + str(int(n)) + str(int(m)) for t, n, m in zip(cv_predsT, cv_predsN, cv_predsM)]

        return cv_predsTNM
    
    def training(self, df, batch_size=32, epochs=10, learning_rate=2e-5, max_length=512):
        train_dataset = MultiLabelDataset(self.tokenizer, texts=df['text'].to_list(), labelT=df['T'].to_list(), labelN=df['N'].to_list(), labelM=df['M'].to_list(), max_length=max_length)
        self.train_loader = DataLoader(dataset=train_dataset, batch_size=batch_size, shuffle=True)
        self.model = BERTMultiLabelModel(model_name=self.model_name).to(self.device)
        self.optimizer = AdamW(self.model.parameters(), lr=learning_rate)
        for epoch in range(epochs):
    ...
    def inferencing(self, df, model, batch_size=32, max_length=512, label2id=None):
        ...
        with torch.no_grad():
            for step, batch in enumerate(test_loader):
                ...
                predsT = np.concatenate((predsT, outputT.cpu().detach().numpy().argmax(axis=1)))
                predsN = np.concatenate((predsN, outputN.cpu().detach().numpy().argmax(axis=1)))
                predsM = np.concatenate((predsM, outputM.cpu().detach().numpy().argmax(axis=1)))
    ...
    def _train_epoch(self):
        train_labelsT, train_labelsN, train_labelsM = np.empty(0), np.empty(0), np.empty(0)
        ...
        for step, batch in enumerate(self.train_loader):
            ...
            train_labelsT = np.concatenate((train_labelsT, labelT.cpu().detach().numpy()))
            train_labelsN = np.concatenate((train_labelsN, labelN.cpu().detach().numpy()))
            train_labelsM = np.concatenate((train_labelsM, labelM.cpu().detach().numpy()))
            train_predsT = np.concatenate((train_predsT, outputT.cpu().detach().numpy().argmax(axis=1)))
            train_predsN = np.concatenate((train_predsN, outputN.cpu().detach().numpy().argmax(axis=1)))
            train_predsM = np.concatenate((train_predsM, outputM.cpu().detach().numpy().argmax(axis=1)))
        ...
        return {'loss': train_loss, 'f1': train_f1, 'accuracy': train_acc, 'labelsT': train_labelsT, 'labelsN': train_labelsN, 'labelsM': train_labelsM, 'predsT': train_predsT, 'predsN': train_predsN, 'predsM': train_predsM}
    ...
    def _eval_epoch(self):
        ...
        with torch.no_grad():
            for step, batch in enumerate(self.eval_loader):
                ...
                eval_labelsT = np.concatenate((eval_labelsT, labelT.cpu().detach().numpy()))
                eval_labelsN = np.concatenate((eval_labelsN, labelN.cpu().detach().numpy()))
                eval_labelsM = np.concatenate((eval_labelsM, labelM.cpu().detach().numpy()))
                eval_predsT = np.concatenate((eval_predsT, outputT.cpu().detach().numpy().argmax(axis=1)))
                eval_predsN = np.concatenate((eval_predsN, outputN.cpu().detach().numpy().argmax(axis=1)))
                eval_predsM = np.concatenate((eval_predsM, outputM.cpu().detach().numpy().argmax(axis=1)))
            ...
            return {'loss': eval_loss, 'f1': eval_f1, 'accuracy': eval_acc, 'labelsT': eval_labelsT, 'labelsN': eval_labelsN, 'labelsM': eval_labelsM, 'predsT': eval_predsT, 'predsN': eval_predsN, 'predsM': eval_predsM}
